...
| # | 改进 | 优先级 | 风险 | 主要收益 |
|---|------|--------|------|----------|
| 1 | 搜索精度（短语/降级/排序/去重）　**部分✅**（去重已做） | **P0** | 低 | 搜得更准、不再空结果 |
| 2 | 检索增强：OpenAlex 选题 + Query Rewriter　**✅ 已实现** | P1 | 低 | 相关性选题、中文/自然语言检索 |
| 3 | LLM 摘要并发（Map 章节级）　**✅ 已实现** | P1 | 中 | 缩短摘要阶段耗时 |
| 4 | MinerU 批处理（目录模式）　**✅ 已实现** | P2 | 高 | 缩短冷启动（已被缓存摊销） |
| 5 | 零散项（见第 5 节） | P3 | 低 | 一致性 / 收尾 |

> 进度（2026-06）：#2 检索增强已落地并扩展（去重合并 / arXiv 标题兜底 / 按意图自动篇数 /
> 改写与摘要的 system 提示词）；#4 MinerU 批处理（目录模式 A 方案）已实现。详见各节 ✅ 标注。
...
### 1.4 版本去重　✅（OpenAlex 路径已做）
- 动机：同一论文多条记录（v1/v2，或预印本/正式版）都返回。
- 已实现：OpenAlex 路径 `_dedup_merge` 按归一化标题合并同篇（见第 2 节）。
- 仍可补：纯 arXiv 路径按 `arxiv_id` 去掉 `vN` 主体去重（`search_agent.py` 的 `_parse_entries`）。
...
### ✅ 已实现并实测（2026-06）
- `agents/query_rewriter.py`：自然语言→英文检索词，复用 DeepSeek（**system 提示词**承载规则）。
  实测 `我想了解大模型如何减少幻觉` → `large language model hallucination reduction`。
  新增两条规则：**干净英文/精确标题原样保留**（修 `transformer` 被改散）、**句中点名某论文则精确提取标题**。失败一律兜底返回原文。
- `agents/openalex_agent.py`：OpenAlex Works API，含 **polite pool(mailto) / 重试 / 429 退避 / 限速 / 缓存（前缀 search_oa_）**；重建倒排 abstract、正则提取 arXiv id。
  排序**默认 `smart`**（相关候选内按引用重排，兼顾相关性+影响力；纯 `citations` 在宽泛词下会顶弱相关高引如 R 语言）。
  **去重合并** `_dedup_merge`：同篇多记录（预印本/正式版）合并为一条——引用取最大、PDF 优选 arXiv 直链（避开反爬出版商链），从源头消重 + 减少下载兜底。
- `agents/search_agent.py`：容错 import + 来源优先级 `OpenAlex > S2 > arXiv`，缓存按来源区分。新增：
  **arXiv 标题兜底**（出版商链下不动时用标题回查 arXiv，结果落盘 `arxiv_title_map.json` 含负缓存、快速失败、反爬域名黑名单跳过首次下载）、**按意图自动篇数** `suggest_max_results`（`search_papers(auto_count=)`）。**契约字段不变（app/UI 不用动）**。
- `agents/llm_agent.py`：`_call_deepseek` 支持可选 `system` 角色；Map/Reduce 摘要各加 system 提示词（身份+纪律）。
...
### 剩余 TODO（组员）
- 设 `OPENALEX_MAILTO`（进 polite pool）—— `start.bat` 已填本机邮箱。
- `openalex_agent.py` 排序/字段精调（smart 已兼顾相关性+引用；可加年份/领域过滤）。
- 可选：`citation_count` 透传到 UI 展示引用数（`search_contract` 可加可选字段）。
  注：界面已显示**全文来源**（arXiv/出版商/仅摘要），由 `views.fulltext_source` 从 pdf_url 推断，不占契约字段。

**验收**：开 `USE_QUERY_REWRITE` + `USE_OPENALEX` 后，中文自然语言进 → OpenAlex 按引用排序选题 → 有 arXiv 版正常解析 → 无 PDF 的退化到摘要；关掉开关回到纯 arXiv。

---

## 3. LLM 摘要并发（P1）　✅ Map 章节级已实现

**动机**：`llm_agent.py` 的 Map 阶段逐章节串行调 DeepSeek，一篇 25~30 节就要 25~30 次串行等待。

### ✅ 已实现（章节级，2026-06）
- `summarize_paper` 的 Map 循环改为 `ThreadPoolExecutor`，章节摘要并发（I/O 密集，收益大）。
  **`executor.map` 按输入顺序返回**，严格保证与 Reduce 的 `[1][2]` 编号对应。
- 并发数环境变量 `MAP_CONCURRENCY`（默认 5，**设 1 = 退回串行**）；有界以尊重 DeepSeek 限流，
  429 由已有指数退避兜底。不动契约、不碰 Streamlit。
- 实测：8 节 mock 4.0x；真实 6 节论文 16.5s→6.4s（2.6x）；冒烟测试加了"顺序严格保持"断言。

### 未做（按需）
- **论文级并发**（多篇 `summarize_paper` 同时跑）：收益与章节级重叠，但 worker 线程**不能调
  `st.*`**（进度条），需重构 app.py 编排，为答辩稳定性暂不做。

---

## 4. MinerU 批处理（P2）　✅ A 方案已实现

**动机**：`pdf_parser.py` 逐篇 `subprocess` 调 `mineru` CLI，**每篇都重新加载模型**（冷启动大头），串行 N 篇 = N 次模型加载；单篇仅占 ~1G 显存，GPU 算力大量闲置。

### ✅ 已实现（A 方案，2026-06）
- `agents/pdf_parser.py` 新增 `parse_pdfs(local_paths)`：命中 md 缓存的复用，未命中的暂存为
  `doc{idx}.pdf` 合并为**一次 `mineru -p <目录> -o <out>`**（目录模式），模型只装载一次，
  按 idx 回映射各自 md。**批量整体失败自动回退逐篇 `parse_pdf`**，保证健壮。
- `contracts/parse_contract.py` 导出 `parse_pdfs`；`app.py` 解析阶段：无 PDF 走摘要兜底、
  有 PDF 合并批量。单篇 `parse_pdf` 契约不变。
- 实测：未缓存的 Informer 经批量入口解析成功（15779 字 / 30 节 / 104s）。

### 进阶（B 方案，未做，高风险）
- **常驻服务 + 并发**：起常驻 mineru API 服务，HTTP 多并发喂满 GPU。需研究 pipeline 后端的
  并发安全性。冷启动已被缓存摊销（热跑 0.4s），课程范围内性价比不高，**建议单独开分支研究**。
